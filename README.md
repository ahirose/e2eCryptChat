...
- **🔍 公開鍵フィンガープリント**: 中間者攻撃を検知するためのSafety Numbers機能
...
- **SHA-256 (Secure Hash Algorithm 256-bit)**
  - 用途: 公開鍵フィンガープリント生成
  - 出力: 256ビット（32バイト）ハッシュ

### 公開鍵フィンガープリント検証（Safety Numbers）

中間者攻撃（MITM）を検知するため、会話相手のIdentity Key（長期公開鍵）のフィンガープリントを表示・検証する機能を実装しています。

#### フィンガープリント生成方法

```
公開鍵 (Base64) → SHA-256ハッシュ → 60桁の数字 → 5桁ごとに区切り
```
...
1. **チャット開始**: 両者がアプリで接続し、フィンガープリントが表示される
2. **帯域外確認**:
   - 電話で読み上げて比較
   - 対面で画面を見せ合う
   - ビデオ通話で画面共有
...
#### Signal Messengerとの比較

このアプリの実装は、Signal MessengerのSafety Numbers機能と同等の仕組みです：

| 項目 | Signal | このアプリ |
|------|--------|----------|
| フィンガープリント形式 | 60桁（12桁×5） | 60桁（5桁×12） |
| ハッシュアルゴリズム | SHA-512 | SHA-256 |
| 表示方法 | QRコード + 数字 | 数字のみ |
| 確認方法 | 帯域外確認推奨 | 帯域外確認必須 |
...
### フィンガープリント検証（中間者攻撃の検知）

会話相手の公開鍵が本物であることを確認し、中間者攻撃を検知する機能です。

#### フィンガープリントの表示

1. **チャット相手を選択**: 左サイドバーから相手をクリック
2. **自動表示**: メッセージ領域の上部に自動的にフィンガープリント表示が現れます
   ```
   ...
   ⚠️ これらの数字を帯域外で確認してください
   ```
3. **折りたたみ**: `−`ボタンで折りたたみ、`+`ボタンで展開可能

#### フィンガープリント検証手順

**重要**: フィンガープリントは必ず**帯域外**（別の通信手段）で確認してください。

1. **電話で確認する場合**:
   ```
   「12345、67890、12345、67890...」
   ```
   - 相手と電話しながら、数字を読み上げて確認
   - 5桁ずつ区切られているので読みやすい
...
- ✅ **一致した場合**: 中間者攻撃を受けていません。安全に通信できます
- ❌ **一致しない場合**:
  - 中間者攻撃の可能性があります
  - すぐにチャットを中止してください
  - ネットワーク管理者やセキュリティ担当者に連絡してください

#### なぜ帯域外確認が必要？

アプリ内のチャットで「フィンガープリントは○○○です」と送信しても、攻撃者がメッセージを改ざんできるため意味がありません。
...
- ✅ **中間者攻撃の検知**: フィンガープリント検証により中間者攻撃を検知可能
...
- ⚠️ **中間者攻撃の自動防止**: フィンガープリント検証は**手動**で行う必要があります
  - 実装済み: フィンガープリント表示機能
  - ユーザーの責任: 帯域外で検証を実施すること
...
2. **身元確認**: ✅ 公開鍵フィンガープリントの検証機能（Safety Numbers）実装済み
   - ⚠️ ユーザーが必ず帯域外で検証を実施するよう、UI/UXで誘導すること
...
**フィンガープリント検証**:
- `generateFingerprint(publicKeyBase64)`: 公開鍵からフィンガープリント生成
  - SHA-256でハッシュ化
  - 60桁の数字（5桁×12グループ）に変換
  - 例: "12345 67890 12345 67890 ..."
- `getOwnFingerprint()`: 自分の公開鍵のフィンガープリント取得
...
**フィンガープリント検証**:
- `updateFingerprintDisplay()`: フィンガープリント表示の更新
  - 自分と相手のフィンガープリントを生成
  - UIに表示（折りたたみ可能）
  - 帯域外確認の重要性を警告メッセージで表示
- `peers Map`: ピア情報に公開鍵を保存（`publicKey`フィールド）
...
- [Key Fingerprint - Wikipedia](https://en.wikipedia.org/wiki/Public_key_fingerprint)
- [Man-in-the-middle attack - Wikipedia](https://en.wikipedia.org/wiki/Man-in-the-middle_attack)
- [Safety Numbers - Signal Support](https://support.signal.org/hc/en-us/articles/360007060632-What-is-a-safety-number-and-why-do-I-see-that-it-changed)
